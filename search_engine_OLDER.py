...
class AdaptiveInventorySearchEngine:
    ...
    def _get_value_for_item(self, item_info):
        ...
        try:
            ...
            logger.debug(f"Getting value for {item_info.get('original', 'unknown')} on sheet '{sheet_name}' row {row}, using column {value_col}")
            
            if not value_col:
                logger.warning(f"No value column found for sheet '{sheet_name}'")
                return 'unknown'
            
            # Get the value
            value = sheet.cell(row=row, column=value_col).value
            logger.debug(f"Read value from sheet[{sheet_name}].cell(row={row}, column={value_col}): {value}")
