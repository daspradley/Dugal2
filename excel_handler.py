...
class ExcelHandler(QWidget):
    ...
    def load_excel_file(self, file_path: str, read_only: bool = True) -> bool:
        ...
        try:
            ...
            # Close any existing workbook BEFORE opening new one
            if self.state.workbook:
                try:
                    logger.debug("Closing previous workbook before loading new file")
                    self.state.workbook.close()
                    self.state.workbook = None
                    import gc
                    gc.collect()  # Force cleanup
                    logger.debug("Previous workbook cleaned up")
                except Exception as e:
                    logger.warning(f"Error closing previous workbook: {e}")
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            # Close workbook with aggressive cleanup
            if self.state.workbook:
                try:
                    self.state.workbook.close()
