...
class AICommandInterpreter:
    ...
    def __init__(
        ...
        if not self.api_key:
            logger.error("⚠️ ANTHROPIC_API_KEY not found! Check your .env file!")
            raise ValueError(
                "Anthropic API key required!\n"
                "Please add ANTHROPIC_API_KEY=sk-ant-... to your .env file"
            )
        
        logger.debug(f"API key loaded successfully (first 20 chars): {self.api_key[:20]}...")
