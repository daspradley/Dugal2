...
class AdaptiveInventorySearchEngine:
    ...
    def __init__(self, learning_file="dugal_patterns.json", logging_manager=None):
        ...
        # Auto-register in global registry if not already present
        try:
            from global_registry import GlobalRegistry
            existing = GlobalRegistry.get('search_engine')
            if not existing:
    ...
    def get_search_engine(self):
        """Get the search engine (self) or from global registry if somehow different."""
        try:
            # First check if this instance is registered in the registry
            from global_registry import GlobalRegistry
            registry_engine = GlobalRegistry.get('search_engine')
            
            # If no engine in registry, register self
            if not registry_engine:
                GlobalRegistry.register('search_engine', self)
                self.logger.debug(f"Registered self in global registry (ID: {id(self)})")
                return self
                
            # If there's a different instance in the registry
            if registry_engine is not self:
                self.logger.warning(f"Different search engine instance in registry (ID: {id(registry_engine)})")
                # For the search engine class, we prioritize self over registry in this method
                # This is different from other components that prioritize the registry
                return self
            ...
            return self  # Always return self for search engine class
    ...
    def _find_inventory_column(self, workbook) -> int:
        """Find the column index for inventory quantity (not cost/price columns)."""
        try:
            logger.debug("Searching for inventory quantity column...")
            
            # Priority list of column names to look for (in order of preference)
            inventory_column_patterns = [
                "ending inventory",
                "current stock", 
                "quantity in stock",
                "stock level",
            ...
            # Exclude patterns that likely contain costs/prices
            exclude_patterns = [
                "cost",
                "price", 
                "value",
            ...
            # Check all sheets for potential inventory columns
            for sheet_name in workbook.sheetnames:
                ...
                for cell in sheet[1]:
                    if cell.value:
                        ...
                        logger.debug(f"Examining header: '{header_text}' at column {cell.column}")
                        
                        # Skip if it contains exclude patterns
                        if any(exclude in header_text for exclude in exclude_patterns):
                            logger.debug(f"Skipping column '{header_text}' - contains cost/price indicators")
                            continue
                        
                        # Check for inventory patterns
                        for pattern in inventory_column_patterns:
                            if pattern in header_text:
                                # Validate this column contains numeric inventory data
                                sample_values = self._validate_inventory_column(sheet, cell.column)
                                if sample_values['is_valid']:
                                    ...
                                    logger.debug(f"Found potential inventory column: '{cell.value}' at column {cell.column}")
                                    break
            
            if found_columns:
                # Sort by priority (first pattern match wins)
                found_columns.sort(key=lambda x: inventory_column_patterns.index(x['pattern_matched']))
                best_column = found_columns[0]
                
                self.input_column_name = best_column['header']
                logger.info(f"Selected inventory column: '{best_column['header']}' at column {best_column['column']} (pattern: {best_column['pattern_matched']})")
                return best_column['column']
            
            # If no inventory patterns found, look for "FARMbar" specifically as fallback
            logger.warning("No standard inventory patterns found, looking for 'FARMbar' specifically...")
            for sheet_name in workbook.sheetnames:
                sheet = workbook[sheet_name]
                for cell in sheet[1]:
                    if cell.value and "farmbar" in str(cell.value).lower():
                        # Validate this is actually inventory data
                        sample_values = self._validate_inventory_column(sheet, cell.column)
                        if sample_values['is_valid']:
                            self.input_column_name = str(cell.value)
                            logger.info(f"Found FARMbar column: '{cell.value}' at column {cell.column}")
                            return cell.column
                        else:
                            logger.warning(f"FARMbar column '{cell.value}' contains non-inventory data: {sample_values}")
            
            logger.error("Could not find any suitable inventory quantity column")
            return None
    ...
    def _validate_inventory_column(self, sheet, column_index) -> dict:
        """Validate that a column contains inventory quantity data (not prices/costs)."""
        try:
            sample_values = []
            numeric_count = 0
            total_count = 0
            
            # Check first 20 data rows (skip header)
            for row in range(2, min(22, sheet.max_row + 1)):
                ...
                if cell_value is not None:
                    ...
                    try:
                        ...
                        # Inventory quantities should typically be:
                        # - Whole numbers or small decimals (not precise cents like 1.149850746268657)
                        # - Not extremely large values (not prices in dollars)
                        # - Usually between 0 and reasonable inventory counts
                        
                    except (ValueError, TypeError):
                        pass
            
            if total_count == 0:
                return {'is_valid': False, 'reason': 'No data found', 'samples': []}
            
            numeric_ratio = numeric_count / total_count if total_count > 0 else 0
            
            # Check if values look like inventory quantities vs prices/costs
            if numeric_count > 0:
                ...
                if numeric_values:
                    ...
                    # Heuristics for inventory vs cost data:
                    # - Inventory: Usually integers or simple decimals, reasonable quantities
                    # - Cost: Often has many decimal places, dollar amounts
                    
                    has_excessive_decimals = any(
                        len(str(v).split('.')[-1]) > 3 for v in numeric_values 
                        if '.' in str(v)
                    )
                    
                    is_valid_inventory = (
                        numeric_ratio > 0.7 and  # Most values are numeric
                        not has_excessive_decimals and  # Not cost calculations with many decimals
                        max_value < 10000  # Reasonable inventory quantity
                    )
                    
                    return {
                        'is_valid': is_valid_inventory,
                        'reason': f'numeric_ratio={numeric_ratio:.2f}, avg={avg_value:.2f}, max={max_value}, excessive_decimals={has_excessive_decimals}',
                        'samples': sample_values[:5]
                    }
            
            return {'is_valid': False, 'reason': f'Low numeric ratio: {numeric_ratio:.2f}', 'samples': sample_values[:5]}
            
        except Exception as e:
            logger.error(f"Error validating column: {e}")
            return {'is_valid': False, 'reason': f'Error: {e}', 'samples': []}

    def build_search_index(self, workbook, search_column: str) -> None:
        """Build search index from workbook with enhanced column detection."""
        logger.debug("Building search index...")
        self.inventory_cache.clear()
        self.workbook = workbook  # Store reference to workbook
        column_idx = column_index_from_string(search_column)
        
        # Find input column index using enhanced detection
        if not hasattr(self, 'input_column_index') or not self.input_column_index:
            ...
            if self.input_column_index:
                logger.info(f"Found inventory column at index {self.input_column_index}: '{self.input_column_name}'")
            else:
                # Only use default if we couldn't find any suitable column
                logger.error("Could not find suitable inventory column, manual selection required")
                raise ValueError("No suitable inventory quantity column found. Please verify your spreadsheet has a proper inventory quantity column.")
